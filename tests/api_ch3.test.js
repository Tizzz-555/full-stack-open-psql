...
describe("Authors API", () => {
  ...
  it("author stats have correct structure", async () => {
    ...
    if (response.data.length > 0) {
      ...
      const blogsCount = author.blogs ?? author.n_blogs;
      const likesCount = author.likes ?? author.n_likes;
      assert.ok(author.author);
      assert.ok(!isNaN(Number(blogsCount)));
      assert.ok(!isNaN(Number(likesCount)));
      assert.ok(Number(blogsCount) >= 0);
      assert.ok(Number(likesCount) >= 0);
    }
  });
});
